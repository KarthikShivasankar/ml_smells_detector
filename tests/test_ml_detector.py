...
def test_data_leakage_detection(tmp_path):
    ...
    smells = detector.detect_smells(str(test_file))
    assert any("Potential data leakage" in smell['smell'] for smell in smells)
...
def test_magic_numbers_detection(tmp_path):
    ...
    smells = detector.detect_smells(str(test_file))
    assert any("Magic number detected" in smell['smell'] for smell in smells)
...
def test_cross_validation_detection(tmp_path):
    ...
    smells = detector.detect_smells(str(test_file))
    assert any("Cross-validation not detected" in smell['smell'] for smell in smells)
